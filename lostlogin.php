...
if ($searchfor != "")
	{
	$lostquery = "select * from earlysignups where id=\"$searchfor\" or email=\"$searchfor\"";
	$lostresult = mysql_query($lostquery);
	$lostrow = mysql_num_rows($lostresult);
	if ($lostrow < 1)
		{
		$show = "Sorry, " . $searchfor . " was not found in the database for any user ID or email address. Please try again or <a href=\"mailto:" . $adminemail . "\">Contact Us</a> for assistance.";
		}
	if ($lostrow > 0)
		{
		$id = mysql_result($lostresult,0,"id");
...
<tr><td align="right">Your User ID# or Email: </td><td><input type="text" name="searchfor" class="typein" maxlength="255" size="25"></td></tr>
</table>
</td></tr>
